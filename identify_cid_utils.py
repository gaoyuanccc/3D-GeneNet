...
def identify_cid(cid_interaction_file,gene_location,resolution):
    ...
    # 获取bin的总数
    num_bins = max(max(k) for k in interaction_dict_log.keys())
    ...
    bin_index_dict = {i: [(i - 1) * resolution, i * resolution] for i in range(1, num_bins + 1)}
    ...
    return CID_boundary,CIDs_corrected,bin_interaction_preference_paired,gene_cid