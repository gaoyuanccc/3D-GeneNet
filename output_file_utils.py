...
def save_cid_range_list(cid_list, output_cid_range_path):
    """
    将CID列表保存到文件中。
...
def save_gene_cid_to_file(data_dict, output_path):
    """
    将字典保存到文件中。
...
def save_dataframe_to_file(data_dataframe, output_path):
    """
    将字典保存到文件中。
...
def plot_cid_interaction_file(bin_interaction_preference_paired,CID_boundary,output_path):
